...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flaglamp_status)
    {
        info->event[info->num] = EVENT_lamp_status;
        info->num++;
        dataPoints->valuelamp_status = gizStandardDecompressionValue(lamp_status_BYTEOFFSET,lamp_status_BITOFFSET,lamp_status_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmist_status)
    {
        info->event[info->num] = EVENT_mist_status;
        info->num++;
        dataPoints->valuemist_status = gizStandardDecompressionValue(mist_status_BYTEOFFSET,mist_status_BITOFFSET,mist_status_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flaghm04_status)
    {
        info->event[info->num] = EVENT_hm04_status;
        info->num++;
        dataPoints->valuehm04_status = gizStandardDecompressionValue(hm04_status_BYTEOFFSET,hm04_status_BITOFFSET,hm04_status_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagtemperature_unit)
    {
        info->event[info->num] = EVENT_temperature_unit;
        info->num++;
        dataPoints->valuetemperature_unit = gizStandardDecompressionValue(temperature_unit_BYTEOFFSET,temperature_unit_BITOFFSET,temperature_unit_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flaglamp_mode)
    {
        info->event[info->num] = EVENT_lamp_mode;
        info->num++;
        dataPoints->valuelamp_mode = gizStandardDecompressionValue(lamp_mode_BYTEOFFSET,lamp_mode_BITOFFSET,lamp_mode_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagscenario)
    {
        info->event[info->num] = EVENT_scenario;
        info->num++;
        dataPoints->valuescenario = gizStandardDecompressionValue(scenario_BYTEOFFSET,scenario_BITOFFSET,scenario_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmist_mode)
    {
        info->event[info->num] = EVENT_mist_mode;
        info->num++;
        dataPoints->valuemist_mode = gizStandardDecompressionValue(mist_mode_BYTEOFFSET,mist_mode_BITOFFSET,mist_mode_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagmist_timer)
    {
        info->event[info->num] = EVENT_mist_timer;
        info->num++;
        dataPoints->valuemist_timer = gizStandardDecompressionValue(mist_timer_BYTEOFFSET,mist_timer_BITOFFSET,mist_timer_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagspeaker_status)
    {
        info->event[info->num] = EVENT_speaker_status;
        info->num++;
        dataPoints->valuespeaker_status = gizStandardDecompressionValue(speaker_status_BYTEOFFSET,speaker_status_BITOFFSET,speaker_status_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
        
    if(0x01 == issuedData->attrFlags.flaglamp_static_color_r)
    {
        info->event[info->num] = EVENT_lamp_static_color_r;
        info->num++;
        dataPoints->valuelamp_static_color_r = gizX2Y(lamp_static_color_r_RATIO,  lamp_static_color_r_ADDITION, issuedData->attrVals.valuelamp_static_color_r); 
    }
        
    if(0x01 == issuedData->attrFlags.flaglamp_static_color_g)
    {
        info->event[info->num] = EVENT_lamp_static_color_g;
        info->num++;
        dataPoints->valuelamp_static_color_g = gizX2Y(lamp_static_color_g_RATIO,  lamp_static_color_g_ADDITION, issuedData->attrVals.valuelamp_static_color_g); 
    }
        
    if(0x01 == issuedData->attrFlags.flaglamp_static_color_b)
    {
        info->event[info->num] = EVENT_lamp_static_color_b;
        info->num++;
        dataPoints->valuelamp_static_color_b = gizX2Y(lamp_static_color_b_RATIO,  lamp_static_color_b_ADDITION, issuedData->attrVals.valuelamp_static_color_b); 
    }
        
    if(0x01 == issuedData->attrFlags.flaglamp_brightness_percent)
    {
        info->event[info->num] = EVENT_lamp_brightness_percent;
        info->num++;
        dataPoints->valuelamp_brightness_percent = gizX2Y(lamp_brightness_percent_RATIO,  lamp_brightness_percent_ADDITION, issuedData->attrVals.valuelamp_brightness_percent); 
    }
        
    if(0x01 == issuedData->attrFlags.flagspeaker_volum_percent)
    {
        info->event[info->num] = EVENT_speaker_volum_percent;
        info->num++;
        dataPoints->valuespeaker_volum_percent = gizX2Y(speaker_volum_percent_RATIO,  speaker_volum_percent_ADDITION, issuedData->attrVals.valuespeaker_volum_percent); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuelamp_status != cur->valuelamp_status)
    {
        GIZWITS_LOG("valuelamp_status Changed\n");
        ret = 1;
    }
    if(last->valuemist_status != cur->valuemist_status)
    {
        GIZWITS_LOG("valuemist_status Changed\n");
        ret = 1;
    }
    if(last->valuehm04_status != cur->valuehm04_status)
    {
        GIZWITS_LOG("valuehm04_status Changed\n");
        ret = 1;
    }
    if(last->valuetemperature_unit != cur->valuetemperature_unit)
    {
        GIZWITS_LOG("valuetemperature_unit Changed\n");
        ret = 1;
    }
    if(last->valuelamp_mode != cur->valuelamp_mode)
    {
        GIZWITS_LOG("valuelamp_mode Changed\n");
        ret = 1;
    }
    if(last->valuescenario != cur->valuescenario)
    {
        GIZWITS_LOG("valuescenario Changed\n");
        ret = 1;
    }
    if(last->valuemist_mode != cur->valuemist_mode)
    {
        GIZWITS_LOG("valuemist_mode Changed\n");
        ret = 1;
    }
    if(last->valuemist_timer != cur->valuemist_timer)
    {
        GIZWITS_LOG("valuemist_timer Changed\n");
        ret = 1;
    }
    if(last->valuespeaker_status != cur->valuespeaker_status)
    {
        GIZWITS_LOG("valuespeaker_status Changed\n");
        ret = 1;
    }
    if(last->valuelamp_static_color_r != cur->valuelamp_static_color_r)
    {
        GIZWITS_LOG("valuelamp_static_color_r Changed\n");
        ret = 1;
    }
    if(last->valuelamp_static_color_g != cur->valuelamp_static_color_g)
    {
        GIZWITS_LOG("valuelamp_static_color_g Changed\n");
        ret = 1;
    }
    if(last->valuelamp_static_color_b != cur->valuelamp_static_color_b)
    {
        GIZWITS_LOG("valuelamp_static_color_b Changed\n");
        ret = 1;
    }
    if(last->valuelamp_brightness_percent != cur->valuelamp_brightness_percent)
    {
        GIZWITS_LOG("valuelamp_brightness_percent Changed\n");
        ret = 1;
    }
    if(last->valuespeaker_volum_percent != cur->valuespeaker_volum_percent)
    {
    ...
    if(last->valueremaining_water_percent != cur->valueremaining_water_percent)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueremaining_water_percent Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(mist_status_BYTEOFFSET,mist_status_BITOFFSET,mist_status_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemist_status);
    gizStandardCompressValue(hm04_status_BYTEOFFSET,hm04_status_BITOFFSET,hm04_status_LEN,(uint8_t *)devStatusPtr,dataPoints->valuehm04_status);
    if(dataPoints->valuetemperature_unit >= temperature_unit_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuetemperature_unit Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(temperature_unit_BYTEOFFSET,temperature_unit_BITOFFSET,temperature_unit_LEN,(uint8_t *)devStatusPtr,dataPoints->valuetemperature_unit);  
    }
    if(dataPoints->valuelamp_mode >= lamp_mode_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuelamp_mode Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(lamp_mode_BYTEOFFSET,lamp_mode_BITOFFSET,lamp_mode_LEN,(uint8_t *)devStatusPtr,dataPoints->valuelamp_mode);  
    }
    if(dataPoints->valuescenario >= scenario_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuescenario Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(scenario_BYTEOFFSET,scenario_BITOFFSET,scenario_LEN,(uint8_t *)devStatusPtr,dataPoints->valuescenario);  
    }
    if(dataPoints->valuemist_mode >= mist_mode_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuemist_mode Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(mist_mode_BYTEOFFSET,mist_mode_BITOFFSET,mist_mode_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemist_mode);  
    }
    if(dataPoints->valuemist_timer >= mist_timer_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuemist_timer Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
        gizStandardCompressValue(mist_timer_BYTEOFFSET,mist_timer_BITOFFSET,mist_timer_LEN,(uint8_t *)devStatusPtr,dataPoints->valuemist_timer);  
    }
    if(dataPoints->valuespeaker_status >= speaker_status_VALUE_MAX)
    {
        GIZWITS_LOG("[ERROR] valuespeaker_status Error , Illegal Overstep\n");
        return -1;
    }
    else
    {
    ...
    devStatusPtr->valuelamp_static_color_g = gizY2X(lamp_static_color_g_RATIO,  lamp_static_color_g_ADDITION, dataPoints->valuelamp_static_color_g); 
    devStatusPtr->valuelamp_static_color_b = gizY2X(lamp_static_color_b_RATIO,  lamp_static_color_b_ADDITION, dataPoints->valuelamp_static_color_b); 
    devStatusPtr->valuelamp_brightness_percent = gizY2X(lamp_brightness_percent_RATIO,  lamp_brightness_percent_ADDITION, dataPoints->valuelamp_brightness_percent); 
    devStatusPtr->valuespeaker_volum_percent = gizY2X(speaker_volum_percent_RATIO,  speaker_volum_percent_ADDITION, dataPoints->valuespeaker_volum_percent); 
    devStatusPtr->valuehumidity = gizY2XFloat(humidity_RATIO,  humidity_ADDITION, dataPoints->valuehumidity); 
    devStatusPtr->valueremaining_water_percent = gizY2X(remaining_water_percent_RATIO,  remaining_water_percent_ADDITION, dataPoints->valueremaining_water_percent); 
